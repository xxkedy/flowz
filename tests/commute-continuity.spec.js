...
test('v4.8.6 build retains commute continuity, conversation quality, and low-load shadowing override', async () => {
  ...
  expect(adapter).toContain('COMMUTE Theme Structure Guard r11:');
  expect(adapter).toContain('WORD WARM-UP must reach five to eight useful words');
  expect(adapter).toContain('MINI DIALOGUE is mandatory');
  expect(adapter).toContain('Target phrase count includes every assistant model, recast, and repeat');
  expect(adapter).toContain('If kedy says No, no, I mean');
  expect(adapter).toContain('never invent XP');
  expect(adapter).toContain('.mode[data-mode-id="commute"]');
  ...
  expect(adapter).toContain("title:'THEME'");
  expect(html).toContain('flowz-v4.8.5-commute-continuity.js?v=4.8.6-r11');
  expect(html).toContain('class="version"');
  expect(html).toContain('data-release="4.8.6-r16"');
  expect(html).toContain('data-release-label="v4.8.6 r16 · 09/10"');
