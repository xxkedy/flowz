...
test('v4.8.6 enforces verified Diary sync wiring for all kedy modes', async () => {
  ...
  expect(adapter).toContain('Never infer that Notion is unavailable');
  expect(adapter).toContain('one to two English diary sentences');
  expect(adapter).toContain('Do not save a long correction list or Coach Assessment');
  expect(adapter).toContain('Never invent a Flowz XP number');
  ...
  expect(adapter).toContain("modeId==='toeic'?'TOEIC':'LIFE TALK'");
  expect(adapter).toContain("{theme:'Schedule changes',phrase:'Has the meeting been moved?'");
  expect(adapter).toContain("{theme:'Recent life and thoughts',phrase:\"Recently, I've been thinking about it.\"");
  expect(core).toContain("number:'4.8.6-r16'");
  expect(core).toContain("label:'v4.8.6 r16 · 09/10'");
  expect(core).toContain("title:'Flowz v4.8.6 · Duo Battle'");
  expect(core).toContain("footer:'✅ Last updated 2026.09.10 · Flowz v4.8.6 r16'");
  expect(adapter).not.toContain('FlowzApp.release.');
  expect(commute).not.toContain('FlowzApp.release.');
  expect(html).toContain('flowz-app.js?v=4.8.6-r16');
  expect(html).toContain('flowz-v4.8.6-diary-sync.js?v=4.8.6-r3');
  expect(html).toContain('v4.8.6 r16 · 09/10');
  expect(entry).toContain("params.set('v','4.8.6-r16')");
