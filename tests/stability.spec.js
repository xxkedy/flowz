...
test('kedy Talk Prep separates PHRASE book navigation from the right-side shuffle', async ({ page }) => {
  ...
  await expect(page.locator('#commute .phrase-item').first()).toContainText("I'm on my way home.");
  await expect(page.locator('#commute')).toContainText("I'm heading to work.");
  await expect(page.locator('#commute')).toContainText('I want to do some shadowing.');
